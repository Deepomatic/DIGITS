...
def create_dummy_regression_dataset(data_path):
    dim = DUMMY_IMAGE_DIM
    count = DUMMY_IMAGE_COUNT
    min_color = 200
    labels = {'red': 0, 'green': 1, 'blue': 2}
    ...
    for i in range(count):
        ...
        img = np.full((dim, dim, 3), pixel, dtype=np.uint8)
        pil_img = PIL.Image.fromarray(img)
        img_path = os.path.join("images", str(i) + '.png')
        output.append(template.format(os.path.join(data_path, img_path), int(pixel[0] != 0), int(pixel[1] != 0), int(pixel[2] != 0)))
        pil_img.save(os.path.join(data_path, img_path))
        if not img_path and idx == 0:
    ...
    with open(os.path.join(data_path, "files.txt"), "w") as fd:
        fd.write("{}\n".format(len(labels)))
        fd.write("\n".join(output))
        fd.write("\n")
    print "{}\n".format(len(labels))
...
def get_dummy_regression_network():
    """
    A very simple network - one fully connected layer
    """
    return \
...
class WebappBaseTest(object):
    ...
    def setUpClass(cls):
        ...
        cls.images_regression = create_dummy_regression_dataset(cls.data_path)
        # Start up the server
    ...
    def tearDownClass(cls):
        ...
        for job_id in cls.created_regression_models:
            cls.delete_model(job_id)  

        shutil.rmtree(cls.data_path)

    @classmethod
    def create_classification_dataset(cls, **data):
        """
        Create a dataset
        Returns the job_id
    ...
    @classmethod
    def create_regression_dataset(cls, **data):
        """
        Create a dataset
        Returns the job_id
        Raises RuntimeError if job fails to create
        ...
        request_json = data.pop('json', False)
        url = '/datasets/images/regression'
        if request_json:
            url += '.json'
    ...
    @classmethod
    def create_quick_regression_dataset(cls, **kwargs):
        """
        ...
        defaults = {
                'method' : 'upload',
                'textfile_filesPath': cls.images_regression[0],
                'textfile_labelsPath': cls.images_regression[1],
                'resize_width': DUMMY_IMAGE_DIM,
                'resize_height' :DUMMY_IMAGE_DIM
                }
        defaults.update(kwargs)

        return cls.create_regression_dataset(**defaults)

    @classmethod
    def create_quick_classification_dataset(cls, **kwargs):
        """
        Creates a simple dataset quickly
        Returns the job_id
        ...
        defaults.update(kwargs)
        return cls.create_classification_dataset(**defaults)        

    @classmethod
    def create_classification_model(cls, **data):
        """
        Create a model
        Returns the job_id
    ...
    @classmethod 
    def create_regression_model(cls, **data):
        """
        Create a model
        Returns the job_id
        Raise RuntimeError if job fails to create
        ...
        """
        if 'model_name' not in data:
            data['model_name'] = 'dummy_model' #HACK

        request_json = data.pop('json', False)
        url = '/models/images/regression'
        if request_json:
            url += '.json'
        ...
        cls.created_regression_models.append(job_id)
        return job_id

    @classmethod
    def create_quick_classification_model(cls, dataset_id, **kwargs):
        """
        ...
        defaults.update(kwargs)
        return cls.create_classification_model(**defaults)

    @classmethod
    def create_quick_regression_model(cls, dataset_id, **kwargs):
        """
        Creates a simple model quickly
        Returns the job_id
        ...
        defaults = {
                'dataset': dataset_id,
                'method': 'custom',
                'custom_network': get_dummy_regression_network(), #HACK
                'batch_size': DUMMY_IMAGE_COUNT,
                'train_epochs': 1,
                'learning_rate' : 0.00001
                }
        defaults.update(kwargs)
        return cls.create_regression_model(**defaults)
...
class TestDatasetClassificationCreation(WebappBaseTest):
    """
    ...
    def test_invalid_folder(self):
        ...
        try:
            job_id = self.create_classification_dataset(
                    method = 'folder',
    ...
    def test_create_json(self):
        ...
        self.create_quick_classification_dataset(json=True)

    def test_create_delete(self):
        """dataset - create, delete"""
        job_id = self.create_quick_classification_dataset()
...
class TestCreatedClassificationDataset(WebappBaseTest):
    """
    Tests on a dataset that has already been created
    """
    @classmethod
    def setUpClass(cls):
        super(TestCreatedClassificationDataset, cls).setUpClass()
        cls.dataset_id = cls.create_quick_classification_dataset()
...
class TestModelClassificationCreation(WebappBaseTest):
    """
    Model creation tests
    """
    @classmethod
    def setUpClass(cls):
        super(TestModelClassificationCreation, cls).setUpClass()
        cls.dataset_id = cls.create_classification_dataset(
                method = 'folder',
                folder_train = cls.data_path,
                resize_width = DUMMY_IMAGE_DIM,
                resize_height = DUMMY_IMAGE_DIM,
    ...
    def test_create_json(self):
        """model - create w/ json"""
        self.create_quick_classification_model(self.dataset_id, json=True)

    def test_create_delete(self):
        """model - create, delete"""
        job_id = self.create_quick_classification_model(self.dataset_id)
    ...
    def test_snapshot_interval_2(self):
        ...
        job_id = self.create_quick_classification_model(self.dataset_id, train_epochs=1, snapshot_interval=0.5)
        assert self.model_wait_completion(job_id) == 'Done', 'create failed'
    ...
    def test_snapshot_interval_0_5(self):
        ...
        job_id = self.create_quick_classification_model(self.dataset_id, train_epochs=4, snapshot_interval=2)
        assert self.model_wait_completion(job_id) == 'Done', 'create failed'
        rv = self.app.get('/models/%s.json' % job_id)
        assert rv.status_code == 200, 'json load failed with %s' % rv.status_code
        content = json.loads(rv.data)
    ...
    def check_select_gpu(self, gpu_index):
        job_id = self.create_quick_classification_model(self.dataset_id, select_gpu=gpu_index)
        assert self.delete_model(job_id) == 200, 'delete failed'
...
class TestModelRegressionCreation(WebappBaseTest):
    """
    Model creation tests
    """
    @classmethod
    def setUpClass(cls):
        super(TestModelRegressionCreation, cls).setUpClass()
        cls.dataset_id = cls.create_regression_dataset(
                method = 'upload',
                textfile_filesPath = cls.images_regression[0],
                textfile_labelsPath = cls.images_regression[1],
                resize_width = DUMMY_IMAGE_DIM,
    ...
    def test_page_model_new(self):
        """new image regression model page"""
        rv = self.app.get('/models/images/regression/new')
        assert rv.status_code == 200, 'page load failed with %s' % rv.status_code
        assert 'New Image Regression Model' in rv.data, 'unexpected page format'

    def test_visualize_network(self):
        """visualize network"""
        rv = self.app.post('/models/visualize-network',
                data = {'custom_network': get_dummy_regression_network()}
                )
        s = BeautifulSoup(rv.data)
        body = s.select('body')
        assert rv.status_code == 200, 'POST failed with %s\n\n%s' % (rv.status_code, body)
    ...
    def test_create_json(self):
        """model - create w/ json"""
        self.create_quick_regression_model(self.dataset_id, json=True)

    def test_create_delete(self):
        """model - create, delete"""
        job_id = self.create_quick_regression_model(self.dataset_id)
        assert self.delete_model(job_id) == 200, 'delete failed'
        assert not self.model_exists(job_id), 'model exists after delete'
    ...
    #         fd.write(self.model_wait_completion(job_id))
    #     assert self.model_wait_completion(job_id) == 'Done', 'create failed'
    #     rv = self.app.get('/models/%s.json' % job_id)
    #     assert rv.status_code == 200, 'json load failed with %s' % rv.status_code
    #     content = json.loads(rv.data)
    ...
    def test_snapshot_interval_0_5(self):
        """model - snapshot_interval 0.5"""
        job_id = self.create_quick_regression_model(self.dataset_id, train_epochs=4, snapshot_interval=2)
        assert self.model_wait_completion(job_id) == 'Done', 'create failed'
        rv = self.app.get('/models/%s.json' % job_id)
        assert rv.status_code == 200, 'json load failed with %s' % rv.status_code
        content = json.loads(rv.data)
    ...
    def check_select_gpu(self, gpu_index):
        job_id = self.create_quick_regression_model(self.dataset_id, select_gpu=gpu_index)
        assert self.delete_model(job_id) == 200, 'delete failed'
...
class TestCreatedModel(WebappBaseTest):
    ...
    def setUpClass(cls):
        ...
        cls.dataset_id = cls.create_quick_classification_dataset()
        assert cls.dataset_wait_completion(cls.dataset_id) == 'Done', 'dataset creation failed'
        cls.model_id = cls.create_quick_classification_model(cls.dataset_id)
...
class TestDatasetModelInteractions(WebappBaseTest):
    ...
    def test_model_with_deleted_database(self):
        ...
        dataset_id = self.create_quick_classification_dataset()
        ...
        try:
            model_id = self.create_quick_classification_model(dataset_id)
        except RuntimeError:
            return
    ...
    def test_model_create_classification_dataset_delete(self):
...
class TestDatasetRegressionCreation(WebappBaseTest):
    """
    Dataset creation tests
    """
    def test_page_dataset_new(self):
        """new image regression dataset page"""
        rv = self.app.get('/datasets/images/regression/new')
        assert rv.status_code == 200, 'page load failed with %s' % rv.status_code
        assert 'New Image Regression Dataset' in rv.data, 'unexpected page format'

    def test_create_json(self):
        """dataset - create w/ json"""
        self.create_quick_regression_dataset(json=True)

    def test_create_delete(self):
        """dataset - create, delete"""
        job_id = self.create_quick_regression_dataset()
        assert self.delete_dataset(job_id) == 200, 'delete failed'
        assert not self.dataset_exists(job_id), 'dataset exists after delete'
    ...
    #     return self.create_regression_dataset(**data)

class TestCreatedRegressionDataset(WebappBaseTest):
    """
    Tests on a dataset that has already been created
    """
    @classmethod
    def setUpClass(cls):
        super(TestCreatedRegressionDataset, cls).setUpClass()
        cls.dataset_id = cls.create_quick_classification_dataset()
        assert cls.dataset_wait_completion(cls.dataset_id) == 'Done', 'dataset creation failed'
...
class TestCreatedRegressionModel(WebappBaseTest):
    """
    Tests on a model that has already been created
    """
    @classmethod
    def setUpClass(cls):
        super(TestCreatedRegressionModel, cls).setUpClass()
        cls.dataset_id = cls.create_quick_regression_dataset()
        assert cls.dataset_wait_completion(cls.dataset_id) == 'Done', 'dataset creation failed'
        cls.model_id = cls.create_quick_regression_model(cls.dataset_id)
        assert cls.model_wait_completion(cls.model_id) == 'Done', 'model creation failed'
    ...
    def test_classify_one(self):
        """created model - classify one -- regression"""
        image_path = self.images['green'][0]
        image_path = os.path.join(self.data_path, image_path)
        with open(image_path) as infile:
            # StringIO wrapping is needed to simulate POST file upload.
        ...
        rv = self.app.post(
                '/models/images/regression/classify_one?job_id=%s' % self.model_id,
                data = {
                    'image_file': image_upload,
                    'show_visualizations': 'y',
                    }
        ...
        body = s.select('body')
        assert rv.status_code == 200, 'POST failed with %s\n\n%s' % (rv.status_code, body)
        assert rv.data.find("green") != -1, 'image misregressed'
