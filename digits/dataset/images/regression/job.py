...
@subclass
class ImageRegressionDatasetJob(ImageDatasetJob):
    """
    A Job that creates an image dataset for a classification network
    """

    def __init__(self, **kwargs):
        super(ImageRegressionDatasetJob, self).__init__(**kwargs)
        self.pickver_job_dataset_image_regression = PICKLE_VERSION

        self.labels_file = None

    def __setstate__(self, state):
        super(ImageRegressionDatasetJob, self).__setstate__(state)

        if self.pickver_job_dataset_image_regression <= 1:
    ...
    @override
    def job_type(self):
        return 'Image Regression Dataset'
