...
class SplitFolderTask(Task):
    ...
    def __init__(self, folder, **kwargs):
        ...
        super(SplitFolderTask, self).__init__(**kwargs)
        self.pickver_task_parsefolder = PICKLE_VERSION

        self.folder = folder
        self.job_dir = kwargs.pop('job_dir', folder)
        if percent_val is None:
            self.percent_val = 0
        else:
            pct = float(percent_val)
    ...
    @override
    def task_arguments(self, **kwargs):
        args = [sys.executable, os.path.join(os.path.dirname(os.path.dirname(digits.__file__)), 'tools', 'torch_tools.py'),
                self.job_dir,
                self.folder,
                str(int(self.percent_val))
               ]
